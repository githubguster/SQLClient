...
UserDB::UserDB(string database_name, SQLError error)
              :SQLiteSQLManagementDB(database_name, error) 
{
    this->init();
}
UserDB::UserDB(SQLiteSQLClient *db)
        :SQLiteSQLManagementDB(db) 
{
    this->init();
}
...
bool UserDB::drop()
{
    ...
    SQLiteSQLManagementDB::remove(TABLE);
    return ret;
}
